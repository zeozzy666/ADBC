...
|		dq - 11/22 updated copyContacts
|		dq - 12/3 removed aa.print()
|		dq - 01/11 updated sendSMS()
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
function copyContacts(pFromCapId, pToCapId) {
    ...
    //Changed to use refContact Number to lookup refcontact, otherwise copies the CAP Contact
    //
    if (pToCapId == null)
        var vToCapId = capId;
    else
    ...
    if (capContactResult.getSuccess()) {
        ...
        for (yy in Contacts) {
			...
			var refContactId = baseContact.getRefContactNumber();
			var referenceContact = null;
			if (refContactId != null)
				referenceContact = aa.people.getPeople(refContactId).getOutput();
				
			if (referenceContact != null)
			{
			finalContact = referenceContact;
			...
			baseContact.setCapID(vToCapId);

            var result = aa.people.createCapContactWithAttribute(baseContact);
            copied++;
...
function loadAddressAttributes(thisArr) {
    ...
    var capAddResult = aa.address.getAddressWithAttributeByCapId(itemCap);
    if (capAddResult.getSuccess())
        var capAddArray = capAddResult.getOutput()
    else
    	{
    ...
    for (i in capAddArray) {
        addAttrObj = capAddArray[i].getAttributes().toArray()
        for (z in addAttrObj)
            {
            thisArr["AddressAttribute." + addAttrObj[z].getB1AttributeName()] = addAttrObj[z].getB1AttributeValue();
	    }
    }
}
...
function scheduleInspectDate(iType, DateToSched) // optional inspector ID.  This function requires dateAdd function
{
    ...
    var schedRes = aa.inspection.scheduleInspection(capId, inspectorObj, aa.date.parseDate(DateToSched), null, iType, "")
...
function scheduleInspection(iType, DaysAhead) // optional inspector ID.  This function requires dateAdd function
{
    ...
    var schedRes = aa.inspection.scheduleInspection(capId, inspectorObj, aa.date.parseDate(dateAdd(null, DaysAhead)), null, iType, "?? ??? ???? ??????? ???????")
...
function addMonthsToDate(startDate, numMonths) {
    ...
    // adjust to correct month - removed 01/29/2011
...
// temporary function to bypass expresion builder bug
...
function copyFees(sourceCapId, targetCapId, activityID) {

    forceInvoice = false;  //follow the fee invoice flag
    forceInvoiceYes = false;

    if (arguments.length > 3) // optional parameter to force invoicing on or off
    {
        forceInvoice = true;
        forceInvoiceYes = arguments[3];
    }
    ...
    for (x in feeA) {
        ...
        if ((forceInvoice && forceInvoiceYes) || (!forceInvoice && thisFee.status == "INVOICED")) {
            addFeeWithExtraData(thisFee.code, thisFee.sched, thisFee.period, thisFee.unit, "Y", targetCapId, activityID, null, activityID)

            //aa.print("We invoiceing the fee " + thisFee.code + " status : " + thisFee.status);
            var feeSeqArray = new Array();
...
function invoiceAllFeesExcept()
	...
	for (var x in feeA)
		...
		if (!invoiceAll && exists(thisFee.accCodeL1,skipArray)) continue;
                logDebug("Fee Found: " + thisFee.code);
...
fees = loadASITable("AE FEES", actCap);
	for (x in fees)
	{
		...
		if(fees[x]["Transaction Type"].fieldValue.equals("Activity Replacement") && (!appTypeString.equals(licenseAmend) || !getAppSpecific("Activity Replacement").toUpperCase().equals("YES"))) continue;
		if(fees[x]["Transaction Type"].fieldValue.equals("Sponsor Amendment") && (!appTypeString.equals(licenseAmend) || !getAppSpecific("Sponsor Amendment").toUpperCase().equals("YES"))) continue;		
		if(fees[x]["Transaction Type"].fieldValue.equals("Trade Name Amendment") && (!appTypeString.equals(licenseAmend) || !getAppSpecific("Trade Name Amendment").toUpperCase().equals("YES"))) continue;		
		if(fees[x]["Transaction Type"].fieldValue.equals("Partners Amendment") && (!appTypeString.equals(licenseAmend) || !getAppSpecific("Partners Amendment").toUpperCase().equals("YES"))) continue;	
		if(fees[x]["Transaction Type"].fieldValue.equals("Capital Amendment") && (!appTypeString.equals(licenseAmend) || !getAppSpecific("Capital Amendment").toUpperCase().equals("YES"))) continue;	
		if(fees[x]["Transaction Type"].fieldValue.equals("Legal Form Amendment") && (!appTypeString.equals(licenseAmend) || !getAppSpecific("Legal Form Amendment").toUpperCase().equals("YES"))) continue;
		if(fees[x]["Transaction Type"].fieldValue.equals("Location Amendment") && (!appTypeString.equals(licenseAmend) || !getAppSpecific("Location Amendment").toUpperCase().equals("YES"))) continue;
		if(fees[x]["Transaction Type"].fieldValue.equals("License Infr. Amendment") && (!appTypeString.equals(licenseAmend) || !getAppSpecific("License Infr. Amendment").toUpperCase().equals("YES"))) continue;
	...
	var amt = fees[x]["Fee Amount"].fieldValue;
		...
		if (getFeeResult.getSuccess())
			{
				...
				feeEntityTitle = feeList[feeNum].getFeeDescription();
					...
					else if (fees[x]["Max Activity"].fieldValue.equals("CHECKED") && fees[x]["Fee Description"].fieldValue.equals(feeEntityTitle)   && amt >= feeList[feeNum].getFee())
						{
	...
	if (getFeeResult.getSuccess())
		...
		for (feeNum in feeList)
		{
			...
			feeEntityTitle = feeList[feeNum].getFeeDescription();
		        ...
		        logDebug(fees[x]["Fee Description"].fieldValue + " AND " + feeEntityTitle + " AND " + fees[x]["Max Activity"].fieldValue + " AND " + amt + " AND " + feeList[feeNum].getFee());
			...
			else if (!fees[x]["Max Activity"].fieldValue.equals("CHECKED") && fees[x]["Fee Description"].fieldValue.equals(feeEntityTitle))
				{
	...
	if (newFeeResult.getSuccess()) 
	{
		...
		newFee.setFeeDescription(fees[x]["Fee Description"].fieldValue);
		newFee.setAccCodeL1(fees[x]["Account Code"].fieldValue);
		newFee.setAccCodeL3(fees[x]["eDirham Code"].fieldValue);
		newFee.setFeeNotes(fees[x]["Approving Entity"].fieldValue)
		newFee.setUdes(fees[x]["Type"].fieldValue);
		//Start SUP-2013-00115-----------------------------------------------------------------------------------------------]
		tsk = aa.workflow.getTask(capId, "Ready For Payment");		
		payMethod = "";
		if(tsk.getSuccess())
		{
			tsk = tsk.getOutput();
			payMethod = tsk.getDisposition();
		}
		if(cap.isCreatedByACA() || payMethod == "Credit Card")
		{
			alreadyAdded = false;
			existingFee = null;
			ccAE = aa.bizDomain.getBizDomain("CREDIT_CARD_FEE_AE").getOutput().toArray();
			for(xx in ccAE)
			{
				existingFees = aa.finance.getFeeItemByFeeCode(capId, "CCFEE_01", "FINAL").getOutput();
				for(fee in existingFees)
				{
					if(existingFees[fee].getFeeSchudle().toUpperCase().equals(fees[x]["Approving Entity"].fieldValue.toUpperCase()))
					{
						alreadyAdded = true;
                                                existingFee = existingFees[fee].getF4FeeItem();
					}
				}

				if (ccAE[xx].getBizdomainValue().equals(fees[x]["Approving Entity"].fieldValue) && !alreadyAdded)
				{					
					ccFee = aa.finance.createFeeItem(capId, fees[x]["Approving Entity"].fieldValue.toUpperCase(), "CCFEE_01", "FINAL", 1);
					if (ccFee.getSuccess()) 
					{
						var ccfeeSeq = ccFee.getOutput();
						var ccnewFee = aa.finance.getFeeItemByPK(capId, ccfeeSeq).getOutput().getF4FeeItem();

						ccnewFee.setFee(amt * 0.0153);
						aa.finance.editFeeItem(ccnewFee);
					}
				}
				else if (ccAE[xx].getBizdomainValue().equals(fees[x]["Approving Entity"].fieldValue) && alreadyAdded &&  (amt * 0.0153) > existingFee.getFee())
				{
					existingFee.setFee(amt * 0.0153);
					aa.finance.editFeeItem(existingFee);
				}				
			}
		}
		//End SUP-2013-00115----------------------------------------------------------------------------------------------------]
		...
		{
			logDebug("Added Fee for approving entity: " + fees[x]["Approving Entity"].fieldValue + " Amount: " + amt + " Qty: " + qty);
		}		
	}
	else
	{
		logDebug("Failed to add AE Fee " + newFeeResult.getErrorMessage() )
	}
	}
}
...
function buildWorkflow() {
	...
	/** Load Cap Data **/
		...
		if (appTypeString.equals("Licenses/License/Renewal/NA")) {
			isRenew = true;
			tType = "Trade License Renewal" ;
			}
		if (appTypeString.equals("Licenses/License/Cancellation/NA")){
			isCancel = true;
			tType = "Trade License Cancellation" ;
			}
	...
	lfTask = "";
	actTask = "";
	lfTask = aa.workflow.getTask(capId, "GenericCommercialActivity").getOutput();
	actTask = aa.workflow.getTask(capId, "GenericCommercialActivity2").getOutput();
	...
	arabicLF = getAppSpecific("Legal Form Arabic", lfCap);
	lfCount = 0;
	for (x in lfEntity) {
		if (lfEntity[x]["Transaction Type"].fieldValue.equals(tType) && 
			lfEntity[x]["Suspended"].fieldValue != "CHECKED" && 
			(lfEntity[x]["Main Area"].fieldValue == "" || lfEntity[x]["Main Area"].fieldValue.equals(capCity))) {
			
			if (lfCount == 0) {
				lfTask.setTaskDescription(legalForm);
				lfTask.setResLangID("ar_AE");
				lfTask.setResTaskDescription(arabicLF);
				
				lfResult = aa.workflow.insertTaskWithResourceData(lfTask,"P");
				if (!lfResult.getSuccess())
				{ logDebug("error " + lfResult.getErrorMessage()); return false; }
				else logDebug("Inserted task: " + legalForm);
				
				lfSub = aa.workflow.insertSubProcess(lfTask,"GENERICSUB3",true);
				if (!lfSub.getSuccess())
				{ logDebug("error " + lfSub.getErrorMessage()); return false; }
				else logDebug("Added sub process to: " + legalForm);
				
				lfCount += 1;
				}

			subSLA =  lfEntity[x]["SLA"].fieldValue;
			enSub = lfEntity[x]["Approving Entity"].fieldValue;
			arSub = aa.bizDomain.getBizDomainByValue("Authorities", enSub, "ar_AE").getOutput().getDispBizdomainValue();
			...
			workflowResult = aa.workflow.getTasks(capId).getOutput();
		
			if (lfEntity[x]["Order"].fieldValue == 1) subName = "Generic1";
			if (lfEntity[x]["Order"].fieldValue == 2) subName = "Generic2";
			if (lfEntity[x]["Order"].fieldValue == 3) subName = "Generic3";
			if (lfEntity[x]["Order"].fieldValue == 4) subName = "Generic4";
			for (x in workflowResult) if (workflowResult[x].getTaskDescription().equals(subName)) {
				subTask = workflowResult[x];
			}
			...
			var systemUserObj = aa.person.getUser(currentUserID).getOutput();
			    if (taskAssign) {
					...
					for (var thisdpt in dpt)
						...
						var m = dpt[thisdpt];
						if (m.getDeptName() != null) {
							if (m.getDeptName().toUpperCase().equals(assignDept.toUpperCase())){
								systemUserObj.setAgencyCode(m.getAgencyCode());
								systemUserObj.setBureauCode(m.getBureauCode());
								systemUserObj.setDivisionCode(m.getDivisionCode());
								systemUserObj.setSectionCode(m.getSectionCode());
			    ...
			    subTask.setTaskDescription(enSub);
				subTask.setResLangID("ar_AE");
				subTask.setResTaskDescription(arSub);
				subTask.setDaysDue(parseInt(subSLA));
				subTask.setAssignedUser(systemUserObj);
				subTask.setActiveFlag("N");
				
				result = aa.workflow.insertTaskWithResourceData(subTask, "P")
			}
		}
		workflowResult = aa.workflow.getTasks(capId).getOutput();
		for (x in workflowResult) {
			if (workflowResult[x].getTaskDescription().equals("Generic1")) {
				aa.workflow.removeTask(workflowResult[x]);
			}
			if (workflowResult[x].getTaskDescription().equals("Generic2")) {
				aa.workflow.removeTask(workflowResult[x]);
			}
			if (workflowResult[x].getTaskDescription().equals("Generic3")) {
				aa.workflow.removeTask(workflowResult[x]);
			}
			if (workflowResult[x].getTaskDescription().equals("Generic4")) {
				aa.workflow.removeTask(workflowResult[x]);
			}
		}
		...
		for (x in myActivities) {
			...
			logDebug("Activity being added: " + actID);
			actCap = aa.cap.getCapID(actID).getOutput();
			arActivityName = getAppSpecific("Arabic Name",actCap).substr(0,99);
			enActivityName = getAppSpecific("English Name",actCap).substr(0,99);
			...
			actCount = 0;
			for (x in actEntity) {
				if (actEntity[x]["Transaction Type"].fieldValue.equals(tType) && 
					actEntity[x]["Suspended"].fieldValue != "CHECKED" && 
					(actEntity[x]["Main Area"].fieldValue == "" || actEntity[x]["Main Area"].fieldValue.equals(capCity))) {
					logDebug("Has Entities");
					if (actCount == 0) {
						actTask.setTaskDescription(enActivityName);
						actTask.setResLangID("ar_AE");
						actTask.setResTaskDescription(arActivityName);
						
						actResult = aa.workflow.insertTaskWithResourceData(actTask,"P");
						if (!actResult.getSuccess())
						{ logDebug("error " + actResult.getErrorMessage()); return false; }
						else logDebug("Added Task: " + enActivityName);
						
						actSub = aa.workflow.insertSubProcess(actTask,"GENERICSUB3",true);
						if (!actSub.getSuccess())
						{ logDebug("error " + actSub.getErrorMessage()); return false; }
						else logDebug("Added Sub process to: " + enActivityName);
						
						actCount += 1;
						}
					
					subSLA = actEntity[x]["SLA"].fieldValue;
					logDebug("SLA: " + subSLA);
					enSub = actEntity[x]["Approving Entity"].fieldValue;
					logDebug("English Sub: " + enSub);
					arSub = aa.bizDomain.getBizDomainByValue("Authorities", enSub, "ar_AE").getOutput().getDispBizdomainValue();
					logDebug("Arabic Sub: " + arSub);
					if (actEntity[x]["DED Approval"].fieldValue == "CHECKED") {
						assignDept = "";
						taskAssign = false;
						}
					else {
						assignDept = actEntity[x]["Approving Entity"].fieldValue;
						taskAssign = true;
						}
					logDebug("assignDept: " + assignDept);
						
					workflowResult = aa.workflow.getTasks(capId).getOutput();
				
					if (actEntity[x]["Order"].fieldValue == 1) subName = "Generic1";
					if (actEntity[x]["Order"].fieldValue == 2) subName = "Generic2";
					if (actEntity[x]["Order"].fieldValue == 3) subName = "Generic3";
					if (actEntity[x]["Order"].fieldValue == 4) subName = "Generic4";
					for (x in workflowResult) if (workflowResult[x].getTaskDescription().equals(subName)) {
						subTask = workflowResult[x];
					}
					logDebug("subTask: " + subTask.getTaskDescription());
					...
					var systemUserObj = aa.person.getUser(currentUserID).getOutput();
						...
						subTask.setTaskDescription(enSub + " - " + actID);
						subTask.setResLangID("ar_AE");
						subTask.setResTaskDescription(arSub);
						subTask.setDaysDue(parseInt(subSLA));
						subTask.setAssignedUser(systemUserObj);
		...
		/** Handle Inspections **/
		if (controlString == "WorkflowTaskUpdateAfter" && isNew && !preInspectionRequired && !bothInspectionRequired) {
		...
		/** Clean Up **/
		allTasks = aa.workflow.getTasks(capId).getOutput();
		for (x in allTasks) {
			if (allTasks[x].getTaskDescription().indexOf("Generic") != -1) aa.workflow.removeTask(allTasks[x])
		}
